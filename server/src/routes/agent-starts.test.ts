...
function app(repository: Partial<Repository>) {
  ...
  target.route(
    '/agent',
    createAgentStartRoutes(repository as Repository, () => now),
  )
...
describe('atomic Agent project start', () => {
  it('creates project, private conversation, task, and durable workspace in one repository call', async () => {
    ...
    const body = (await response.json()) as {
      ...
      conversation: {
        ...
        projectId: string
        visibility: string
        tasks: Array<{ stages: Array<{ id: string }> }>
      }
    ...
    expect(body.conversation.tasks[0]?.stages.map(stage => stage.id)).toEqual([
      'understand-requirements',
      'plan-layout',
      'bind-data',
      'preview-check',
    ])
    expect(body.workspace).toMatchObject({ revision: 1, payload: { conversations: [body.conversation] } })
  ...
  it('pauses the initial task until declared project-bound uploads are attached', async () => {
    ...
    expect(response.status).toBe(201)
    const body = (await response.json()) as {
      conversation: { messages: Array<{ attachments: unknown[] }>; tasks: Array<{ stages: unknown[] }> }
    }
    expect(body.conversation.messages[0]?.attachments).toEqual([])
    expect(body.conversation.tasks[0]?.stages[1]).toMatchObject({
      id: 'plan-layout',
      status: 'waiting',
      detail: '等待附件上传',
    })
