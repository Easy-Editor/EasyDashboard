...
describe('editor panel layout contracts', () => {
  it('keeps the compact tool rail icon-only without losing accessible names', async () => {
    ...
    expect(source).toContain('groupStart: true')
    expect(source).toContain("item.groupStart && 'mt-1.5 border-t border-[var(--ed-line)] pt-2.5'")
  ...
  it('uses one Phase 0 surface hierarchy across rails, panels, and the canvas world', async () => {
    ...
    expect(configureSource).toContain('SlidersHorizontal')
    expect(configureSource).not.toContain('rounded-full bg-[var(--ed-cyan)]')
    ...
    expect(headerSource).toContain("aria-label='工作模式'")
    expect(headerSource).toContain('Agent 创作')
    expect(headerSource).toContain('手动编辑')
    expect(headerSource).toContain('Agent 面板')
    expect(headerSource).toContain('bg-[var(--ed-ink-soft)]')
    ...
    expect(stylesheet).toContain('--ed-artboard: #0a1017;')
    expect(stylesheet).toContain('--ruler-grid-color: color-mix(in srgb, var(--ed-line) 26%, transparent);')
    ...
    expect(overrides).toContain('background: var(--ed-artboard) !important;')
    expect(overrides).toContain('color-mix(in srgb, var(--ed-line-strong) 82%, transparent)')
