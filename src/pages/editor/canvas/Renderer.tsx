...
const Renderer = observer(() => {
  ...
  useEffect(() => {
    if (!isReady) return
    const timeoutId = window.setTimeout(handleFitWidth, 200)
    return () => window.clearTimeout(timeoutId)
  }, [handleFitWidth, isReady])

  useEffect(() => {
    return project.onRendererReady(() => {
      setIsReady(true)
    })
  }, [])
  ...
  // 模式切换时触发自适应画布
  useEffect(() => {
    const shouldFit = isReady && prevModeRef.current !== mode && mode !== 'code'
    prevModeRef.current = mode
    if (!shouldFit) return
    const timeoutId = window.setTimeout(handleFitWidth, 100)
    return () => window.clearTimeout(timeoutId)
  }, [handleFitWidth, mode, isReady])
    ...
    return (
      ...
      >
        <div className='flex flex-col items-center gap-4 text-muted-foreground'>
          ...
          <svg viewBox='0 0 70 60' width='116' height='100' xmlns='http://www.w3.org/2000/svg' className='opacity-40'>
          ...
          <p className='text-sm'>暂无页面，请先创建页面</p>
        </div>
      </section>
    )
  }
  ...
  return (
    <RendererContextMenu>
      <section
        ...
        className='flex min-w-0 w-full h-full flex-col'
      >
