import Root from './root/meta'

// Inner
import Group from './inner/group/meta'

// Basic
import Image from './basic/image/meta'
import Text from './basic/text/meta'

// Chart
import AreaChart from './chart/area-chart/meta'
import BarChartHorizontal from './chart/bar-chart-horizontal/meta'
import BarChartNegative from './chart/bar-chart-negative/meta'
import BarChartStacked from './chart/bar-chart-stacked/meta'
import BarChart from './chart/bar-chart/meta'
import LineChart from './chart/line-chart/meta'
import PieChartDonut from './chart/pie-chart-donut/meta'
import PieChartStacked from './chart/pie-chart-stacked/meta'
import PieChart from './chart/pie-chart/meta'
import RadarChartLines from './chart/radar-chart-lines/meta'
import RadarChart from './chart/radar-chart/meta'
import RadialChartStacked from './chart/radial-chart-stacked/meta'
import RadialChartText from './chart/radial-chart-text/meta'
import RadialChart from './chart/radial-chart/meta'

// Data
import Carousel from './data/carousel/meta'
import Progress from './data/progress/meta'
import Table from './data/table/meta'

// Interaction
import Button from './interaction/button/meta'
import CalendarButton from './interaction/calendar-button/meta'
import Calendar from './interaction/calendar/meta'
import Combobox from './interaction/combobox/meta'
import Input from './interaction/input/meta'
import Select from './interaction/select/meta'
import Sonner from './interaction/sonner/meta'
import ToggleGroup from './interaction/toggle-group/meta'
import Toggle from './interaction/toggle/meta'

/**
 * 物料元数据
 */
export const componentMetaMap = {
  AreaChart,
  BarChart,
  BarChartHorizontal,
  BarChartNegative,
  BarChartStacked,
  Button,
  Calendar,
  CalendarButton,
  Carousel,
  Combobox,
  Image,
  Input,
  LineChart,
  PieChart,
  PieChartDonut,
  PieChartStacked,
  Progress,
  RadarChart,
  RadarChartLines,
  RadialChart,
  RadialChartStacked,
  RadialChartText,
  Root,
  Select,
  Sonner,
  Table,
  Text,
  Toggle,
  ToggleGroup,
  Group,
}
