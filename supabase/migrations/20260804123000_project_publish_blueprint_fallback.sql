alter table app.project_preview_runs
  ...
  drop constraint project_preview_runs_source_evidence_check,
  add constraint project_preview_runs_source_check check (
    source in (
      'agent_executor',
      'owner_live_render_attestation',
  ...
  add constraint project_preview_runs_source_evidence_check check (
    ...
    or (
      source in ('editor_renderer_artifact', 'editor_blueprint_artifact')
      and agent_operation_id is null
    ...
    or (
      ...
      and exists (
        ...
        where artifact.id = project_preview_runs.thumbnail_artifact_id
          ...
          and (
            (project_preview_runs.source = 'editor_renderer_artifact'
              and artifact.source = 'renderer'
              and artifact.content_type = 'image/webp')
            or
            (project_preview_runs.source = 'editor_blueprint_artifact'
              and artifact.source = 'blueprint'
              and artifact.content_type = 'image/svg+xml')
          )
...
with check (
  ...
  and exists (
    ...
    where preview.id = preview_run_id
      ...
      and preview.source in ('agent_executor', 'editor_renderer_artifact', 'editor_blueprint_artifact')
  )
);
